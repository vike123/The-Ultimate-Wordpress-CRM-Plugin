 ...
 *
 * EDITED BY WILL WILSON DO NOT UPDATE THIS CLASS OR ANY OF IT'S FILES!!!!!!!!
 */
	...
	class VTP_Meta_Box {
		...
		function insert_images() {
			...
			foreach ($_POST['attachments'] as $attachment_id => $attachment) {
				...
				$li .= "<a title='Delete this image' class='rw-delete-file' href='#' rel='$nonce|$post_id|$id|$attachment_id'>'Delete'</a>";
		...
		function show() {
			...
			wp_nonce_field(basename(__FILE__), 'VTP_Meta_Box_nonce');
		...
		function show_field_file($field, $meta) {
			...
			if (!empty($meta)) {
				$nonce = wp_create_nonce('rw_ajax_delete');
				echo '<div style="margin-bottom: 10px"><strong>Uploaded files</strong></div>';
				echo '<ol class="rw-upload">';
				foreach ($meta as $att) {
					// if (wp_attachment_is_image($att)) continue; // what's image uploader for?
					echo "<li>" . wp_get_attachment_link($att, '' , false, false, ' ') . " (<a class='rw-delete-file' href='#' rel='$nonce|{$post->ID}|{$field['id']}|$att'>Delete</a>)</li>";
				}
				echo '</ol>';
			}
	
			// show form upload
			echo "<div style='clear: both'><strong>Upload new files</strong></div>
				<div class='new-files'>
					<div class='file-input'><input type='file' name='{$field['id']}[]' /></div>
					<a class='rw-add-file' href='#'>Add another file</a>
				</div>
			</td>";
		}
		...
		function show_field_image($field, $meta) {
			...
			if (!empty($meta)) {
				...
				foreach ($images as $image) {
					...
					echo "<li id='item_$image'>
							<img src='$src' />
							<a title='Delete this image' class='rw-delete-file' href='#' rel='$nonce_delete|{$post->ID}|{$field['id']}|$image'>Delete</a>
			...
			echo "<a href='#' class='rw-upload-button button' rel='{$post->ID}|{$field['id']}'>Add more images</a>";
		}
		...
		function show_field_color($field, $meta) {
			...
			echo "<input class='rw-color' type='text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='8' />
				  <a href='#' class='rw-color-select' rel='{$field['id']}'>Select a colour</a>
		...
		function save($post_id) {
			...
			|| (!check_admin_referer(basename(__FILE__), 'VTP_Meta_Box_nonce'))		// verify nonce
			...
			foreach ($this->_fields as $field) {
				...
				// validate meta value
				if (class_exists('VTP_Meta_Box_Validate') && method_exists('VTP_Meta_Box_Validate', $field['validate_func'])) {
					$new = call_user_func(array('VTP_Meta_Box_Validate', $field['validate_func']), $new);
				}
